...
fu! s:Define_syntax()
	...
	echo 'syn match Tf_tok /'.b:txtfmt_re_any_tok.'/'
	...
	if !b:txtfmt_cfg_conceal
		...
		while pi <= (b:txtfmt_cfg_bgcolor ? b:txtfmt_cfg_numbgcolors : 0)
			...
			echo 'syn match Tf'.cui.'_tok_'.i.' /'.b:txtfmt_re_any_tok.'/ contained'.conceal
			exe 'syn match Tf'.cui.'_tok_'.i.' /'.b:txtfmt_re_any_tok.'/ contained'.conceal
			echo 'hi Tf'.cui.'_tok_'.i.' '.eq_bgc.b:txtfmt_bgc{i}.eq_clr.b:txtfmt_bgc{i}
			...
			" distinct namespaces.
			echo 'syn cluster Tf'.cui.'_tok add=Tf'.cui.'_tok_'.i
