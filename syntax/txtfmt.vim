...
fu! s:Define_syntax()
	...
	" Last update: Sun Jul 31 18:27:29 2016
	...
	while ip <= b:txtfmt_cfg_numfgcolors
		...
		let rgn_body = skip
			...
			\.' nextgroup=@Tf'.cui.'_clr_all'.(bgc_enabled ? ',@Tf'.cui.'_clrbgc_'.i.'_all' : '').',@Tf'.cui.'_clrfmt_'.i.'_all,Tf_conceal'
	...
	while ip <= b:txtfmt_cfg_numbgcolors
		...
		let rgn_body = skip
			...
			\.' nextgroup=@Tf'.cui.'_bgc_all'.(clr_enabled ? ',@Tf'.cui.'_bgcclr_'.i.'_all' : '').',@Tf'.cui.'_bgcfmt_'.i.'_all,Tf_conceal'
		" Define region that is begun by a start token
	...
	while i <= b:txtfmt_num_formats - 1
		...
		let rgn_body = skip
			...
			\.' nextgroup=@Tf'.cui.'_fmt_all'.(clr_enabled ? ',@Tf'.cui.'_fmtclr_'.i.'_all' : '').(bgc_enabled ? ',@Tf'.cui.'_fmtbgc_'.i.'_all' : '').',Tf_conceal'
		" Define region that is begun by a start token
